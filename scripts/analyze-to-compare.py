...
def process_json(json_file, dependency_type):
    counts = {'total': 0, 'super-linear': 0, 'linear': 0, 'supra-linear': 0, 'bursty': 0}
...
def find_files(directory,path):
    ...
    full_directory = os.path.join(parent_directory, path, directory)
...
if __name__ == "__main__":
    ...
    path = sys.argv[2]
    json_file, csv_file = find_files(directory_name,path)
    ...
    # print('Total count of entries: {} ({})'.format(counts['total'], counts['total']/csv_line_count))
    # print('Super-linear count: {} ({})'.format(counts['super-linear'], counts['super-linear']/counts['total']))
    # print('Linear count: {} ({})'.format(counts['linear'], counts['linear']/counts['total']))
    # print('Supra-linear count: {} ({})'.format(counts['supra-linear'], counts['supra-linear']/counts['total']))

    # print('Bursty count: {} ({})'.format(counts['bursty'], counts['bursty']/counts['total']))
    
    # Our Results
    
    # Modify the output format of the results
print("\nResults Summary: " + f"Total number of loops found in CSV file: {csv_line_count}")
print("=" * 80)
print("{:<20} {:<20} {:<20}".format("Metric", "Count", "Percentage"))
print("-" * 80)
print("{:<20} {:<20} {:<20.2%}".format("Total", counts['total'], counts['total'] / csv_line_count))
print("{:<20} {:<20} {:<20.2%}".format("Super-linear", counts['super-linear'], counts['super-linear'] / counts['total']))
print("{:<20} {:<20} {:<20.2%}".format("Linear", counts['linear'], counts['linear'] / counts['total']))
print("{:<20} {:<20} {:<20.2%}".format("Supra-linear", counts['supra-linear'], counts['supra-linear'] / counts['total']))
print("=" * 80)