...
if __name__ == '__main__':
    df_all_bug_reports = read_bug_reports('scalability-bugs.csv')
    ...
    # table 1: dimensions, summary and examples
    print_msg_box('Dimensions, Summary and Examples', title='Table 1')
    dimensional_tags = ['sc-load', 'sc-data', 'sc-clus', 'sc-fail']
    for tag in dimensional_tags:
        print('{}: {} bugs -> {}%.'.format(tag, len(find_all_bugs_with_tag(df_all_bug_reports, tag)), round(len(find_all_bugs_with_tag(df_all_bug_reports, tag)) / len(df_all_bug_reports) * 100, 2)))
    ...
    rc_unbound_tags.remove('rc-unbound-queue')
    rc_unbound_types = {
        'tt': rc_unbound_tags,
        'unbound-collection': ['rc-unbound-collection'],
        'unbound-allocation': ['rc-unbound-alloc'],
